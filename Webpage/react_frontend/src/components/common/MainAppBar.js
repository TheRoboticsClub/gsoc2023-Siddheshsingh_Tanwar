...
function MainAppBar(props) {
  ...
  const {
    projectURL,
  } = React.useContext(NewManagerExerciseContext);

  const slicedPath = (projectURL.slice(11)).split("/").map((segment) => {
  const words = segment.split(/(?=[A-Z])/);
  return words.join(" ");
  }).join(" - ");
    ...
    <RoboticsTheme>
      <AppBar position="static">
        ...
        >
          ...
          <Typography variant="h5">VisualCircuit</Typography>
          <Box> </Box>
          <Box> {slicedPath}</Box>
